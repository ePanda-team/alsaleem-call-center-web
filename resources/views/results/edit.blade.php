...
<div class="max-w-2xl bg-white rounded-lg border border-gray-200 shadow p-6">
  <form method="post" action="{{ route('results.update', $result) }}" enctype="multipart/form-data" class="space-y-4">
      ...
      <div>
        ...
        <input type="number" min="0" max="150" name="patient_age" class="border rounded px-3 py-2 w-full focus:outline-none focus:ring-2 focus:ring-[#FE0003]" value="{{ old('patient_age', $result->patient_age) }}" />
      </div>
      <div>
