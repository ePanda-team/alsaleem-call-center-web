...
<div class="container mx-auto px-4">
    <div class="max-w-2xl mx-auto">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">{{ __('messages.add_test_result') }}</h1>
            ...
            <form method="post" action="{{ route('results.store') }}" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <select id="lab_branch" name="lab_branch" required
                        ...
                        @foreach($labBranches as $labBranch)
                            <option value="{{ $labBranch->name }}" {{ old('lab_branch') == $labBranch->name ? 'selected' : '' }}>
                                {{ $labBranch->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-6">
                    <label for="pdf" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('messages.pdf_file') }} <span class="text-red-500">*</span></label>
